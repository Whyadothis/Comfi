...
if (interaction.isChatInputCommand()) {
    ...
    await interaction.reply({ content: "Comfi™ is thinking", ephemeral: false }).catch(() => {})
    ...
    const userp = new EmbedBuilder()
      .setDescription(
        `${bot.error} You need \`${cmd.userperm ? cmd.userperm : []}\` Permissions`
      )
      .setColor(0xFC7C7C)

        console.log(cmd.userperm[0].toLowerCase())
    ...
    if (clientSchema.blackListedUsers.includes(interaction.user.id)) {
      const blkuser = new EmbedBuilder()
        .setDescription(
    ...
    if (clientSchema.blackListedServers.includes(interaction.guild.id)) {
      const blkguild = new EmbedBuilder()
        .setDescription(
